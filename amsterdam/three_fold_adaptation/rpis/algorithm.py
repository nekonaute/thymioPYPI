...
class Simulation(threading.Thread):
	...
	def __runAndEvaluate(self, evaluee, maxA, minA, maxR, minR, maxD, minD):
		# Recovery period (tau timesteps)
		self.__simLogger.info("Recovery period")
		for i in range(0, pr.tau):
			(fit, maxA, minA, maxR, minR, maxD, minD, deltaL, deltaR) = self.__runAndEvaluateForOneTimeStep(evaluee, maxA, minA, maxR, minR, maxD, minD)
		...
		for i in range(0, pr.evaltime):
			(f, maxA, minA, maxR, minR, maxD, minD, deltaL, deltaR) = self.__runAndEvaluateForOneTimeStep(evaluee, maxA, minA, maxR, minR, maxD, minD)
			fitness += f
			# avgDeltaL += deltaL
			# avgDeltaR += deltaR
			# if deltaL < 700:
			# 	countL += 1
			# 	if countL >= 50:
			# 		# self.__simLogger.critical("Timestep " + str(i) + " avgDeltaL = " + str(float(avgDeltaL) / (i+1)))	
			# 		raise Exception("Timestep " + str(i) + " avgDeltaL = " + str(float(avgDeltaL) / (i+1)))	
			# if deltaR < 700:
			# 	countR += 1
			# 	if countR >= 50:
			# 		# self.__simLogger.critical("Timestep " + str(i) + " avgDeltaR = " + str(float(avgDeltaR) / (i+1)))
			# 		raise Exception("Timestep " + str(i) + " avgDeltaR = " + str(float(avgDeltaR) / (i+1)))
		return (fitness, maxA, minA, maxR, minR, maxD, minD) # fitness IN [-175000, 175000]

	def __runAndEvaluateForOneTimeStep(self, evaluee, maxA, minA, maxR, minR, maxD, minD):
		...
		# Read ground sensors: request to ThymioController
		self.__thymioController.readGroundSensorsRequest()
		self.__waitForControllerResponse()
		(groundAmbiant, groundReflected, groundDelta) = self.__thymioController.getGroundSensorsValues()
		deltaL = groundDelta[0]
		deltaR = groundDelta[1]
		# if deltaL < 700:
		# 	self.__simLogger.critical("groundDelta[0] = " + str(groundDelta[0]) + " = " + str(groundReflected[0]) + " - " + str(groundAmbiant[0]))
		# 	# raise Exception("groundDelta[0] = " + str(groundDelta[0]) + " = " + str(groundReflected[0]) + " - " + str(groundAmbiant[0]))
		# if deltaR < 700:
		# 	self.__simLogger.critical("groundDelta[1] = " + str(groundDelta[1]) + " = " + str(groundReflected[1]) + " - " + str(groundAmbiant[1]))
		# 	# raise Exception("groundDelta[1] = " + str(groundDelta[1]) + " = " + str(groundReflected[1]) + " - " + str(groundAmbiant[1]))
		for a in groundAmbiant:
			if a > maxA:
				maxA = a
			if a < minA:
				minA = a
		for r in groundReflected:
			if r > maxR:
				maxR = r
			if r < minR:
				minR = r
		for d in groundDelta:
			if d > maxD:
				maxD = d
			if d < minD:
				minD = d
		...
		motorspeed = [0, 0]
		motorspeed[LEFT] = int(left * classes.MAXSPEED)
		motorspeed[RIGHT] = int(right * classes.MAXSPEED)
		...
		if motorspeed[LEFT] > motorspeed[RIGHT]:
			speedpenalty = float((motorspeed[LEFT] - motorspeed[RIGHT])) / float(classes.MAXSPEED)
		else:
			speedpenalty = float((motorspeed[RIGHT] - motorspeed[LEFT])) / float(classes.MAXSPEED)
		if speedpenalty > 1:
			speedpenalty = 1
		...
		fitness = float(motorspeed[LEFT] + motorspeed[RIGHT]) * (1 - speedpenalty) * (1 - sensorpenalty)
		return (fitness, maxA, minA, maxR, minR, maxD, minD, deltaL, deltaR)
	...
	def run(self):
		...
		while (evals < pr.total_evals):
			...
			try:
				...
				(champion.fitness, maxA, minA, maxR, minR, maxD, minD) = self.__runAndEvaluate(copy.deepcopy(champion), maxA, minA, maxR, minR, maxD, minD)
				...
				for l in range(lifetime):
					...
					if (random.random() <= 0.2):
						...
						(re_fitness, maxA, minA, maxR, minR, maxD, minD) = self.__runAndEvaluate(copy.deepcopy(champion), maxA, minA, maxR, minR, maxD, minD) # XXX: why deepcopy??? Challenger is not changed during evaluation
						...
						if (pr.collected_memomes_total > 0 and pr.sociallearning == 1 and random.random() <= 0.3):
							...
							for i in range(classes.NMBRWEIGHTS):
								if (socialChallenger.memome[i] != 100.0 and lastCollectedMemome[i] != 100.0):
									# If sensor is enabled on both champion and lastCollectedMemome -> overwrite value on champion
									socialChallenger.memome[i] = lastCollectedMemome[i]
							pr.collected_memomes_total -= 1

							(socialChallenger.fitness, maxA, minA, maxR, minR, maxD, minD) = self.__runAndEvaluate(copy.deepcopy(socialChallenger), maxA, minA, maxR, minR, maxD, minD) # XXX: why deepcopy??? Challenger is not changed during evaluation
							...
							if pr.lifetimelearning == 1:
								...
								(challenger.fitness, maxA, minA, maxR, minR, maxD, minD) = self.__runAndEvaluate(copy.deepcopy(challenger), maxA, minA, maxR, minR, maxD, minD) # XXX: why deepcopy??? Challenger is not changed during evaluation
		...
		self.__simLogger.info("AMBIANT_MAX = " + str(maxA))
		self.__simLogger.info("ambiant_min = " + str(minA))
		self.__simLogger.info("REFLECTED_MAX = " + str(maxR))
		self.__simLogger.info("reflected_min = " + str(minR))
		self.__simLogger.info("DELTA_MAX = " + str(maxD))
		self.__simLogger.info("delta_min = " + str(minD))
...
class ThymioController(object):
	...
	def __dbusError(self, e):
		...
		self.__simLogger.critical('Error in ThymioController: ' + str(sys.exc_info()[0]) + ' - ' + traceback.format_exc())
