...
class Genome:
	...
	def __init__(self, mainLogger, size=0, geneValue=[]) :
		...
		if size==0:
			...
			if geneValue!=[]:
				...
				# generation d'un genome de taille size avec genes compris entre 0 et 1
				self.gene = [-1+random.random()*2 for i in range(size)] 
				self.size=size
	
	def sigmoide(self,x):
		
		return (1/(1+ math.exp(-4*x))) * Params.maxSpeedValue *2  - Params.maxSpeedValue
	...
	def mutationGaussienne(self):
		...
		for i in range (len(self.gene)) :
			copyGenome.gene[i] = np.random.normal(copyGenome.gene[i], Params.sigma)
