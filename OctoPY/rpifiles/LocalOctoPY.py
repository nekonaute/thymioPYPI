...
LOCALHOST = ip_address(u'127.0.0.1')
LISTENER_HOST = ''
LISTENER_PORT = 55555
...
def sendMessage(message, data = None) :
	# Message
	message = int(message)

	myIP = 127.0.0.1
	destIP = LOCALHOST
	logging.info("sendMessage - sending message " + str(message))
	...
	try :
		...
		if message == MessageType.INIT :
			proc = subprocess.Popen(["asebamedulla", "ser:device=/dev/ttyACM0", "&", "python", os.path.join(THYMIO_SCRIPTS_PATH, 'MainController.py'), '-d', , '-D', '-i', str(myIP), '&'], stdout = subprocess.PIPE, stderr = subprocess.PIPE)
		...
		logging.error("sendMessage - Unexpected error while sending message " + str(message) + " : " + str(sys.exc_info()[0]) + " - " + traceback.format_exc()) 
		continue
...
if __name__ == '__main__' :
	...
	parser.add_argument('-s', '--send', default = None, nargs = '+', help = "Send a message to the thymio")
	parser.add_argument('-q', '--query', default = None, nargs = '*', help = "Query the state of the thymio")
